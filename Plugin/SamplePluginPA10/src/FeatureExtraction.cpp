...
void ColorSegmentation::plop() {
        std::cout<<"plopSegmentation\n";
}
void ColorSegmentation::set( double threshold, double compactness, int dilation, int erosion, cv::Size gaussian, cv::Scalar red_lB, cv::Scalar red_uB, cv::Scalar blue_lB, cv::Scalar blue_uB) {
...
cv::Mat ColorSegmentation::FindCircles(
        const cv::Mat& circles_hsv_image,
...
cv::Mat ColorSegmentation::drawResult(
        const cv::Mat& src,
        std::vector<cv::Point> center) {
...
std::vector<cv::Point> ColorSegmentation::algoFind3Points(
        const std::vector<cv::Point> center) {
